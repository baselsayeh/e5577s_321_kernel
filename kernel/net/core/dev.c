...
/*=======================================================================
  ...
  =======================================================================*/
    ...
    || defined(BSP_CONFIG_BOARD_E5577S_321) || defined(BSP_CONFIG_BOARD_401HW)\
    ...
    || defined(BSP_CONFIG_BOARD_E5573CS) || defined(BSP_CONFIG_BOARD_506HW) \
    || defined(BSP_CONFIG_BOARD_506HW_2) || defined(BSP_CONFIG_BOARD_E5573BS)\
    || defined(BSP_CONFIG_BOARD_E5573CS_933)||defined(BSP_CONFIG_BOARD_E5577S_324)\
...
another_round:
	...
	if (skb->protocol == cpu_to_be16(ETH_P_8021Q)) {
		...
		if (unlikely(!skb))
			goto out;
#ifdef CONFIG_ATP_SMUX
        /*找不到相应VLAN设备，需进入SMUX处理，寻找虚出来的子接口*/
        if (!skb->dev) {
            skb->dev = orig_dev;
...
static void bind_mark(struct sk_buff* skb)
{
    if (IS_LAN_DEV(skb->dev->name))   /* 判断是否为LAN口 */
    {
