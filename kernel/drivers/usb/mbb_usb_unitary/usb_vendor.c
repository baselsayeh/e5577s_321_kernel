...
#ifdef MBB_CHARGE
extern USB_ULONG wall_chg_insert_time ;
extern USB_ULONG wall_chg_remove_time ;
#endif /*MBB_CHARGE*/
...
static void bsp_usb_pmu_insert_detect(void* ctx)
{
    ...
    #endif
    bsp_host_pmu_status_update(USB_BALONG_HOSTPMU_INSERT);
    /* when otg-host supply vbus,pmu intr is obmit */
    printk("pre pmu insert event received\n");
    if(bsp_usb_is_support_vbus_bycharger())
    {
        if(USB_BALONG_PERIP_INSERT == usb_get_hotplug_old_status())
        {   
            bsp_host_pmu_status_change(USB_BALONG_HOSTPMU_INSERT);
            return ;
        }
    }
    ...
    {
        if(GPIO_LOW == gpio_get_value(GPIO_OTG_ID_DET))
        {
            bsp_host_pmu_status_change(USB_BALONG_HOSTPMU_INSERT);
...
static void bsp_usb_pmu_remove_detect(void* ctx)
{
    ...
    #endif
    bsp_host_pmu_status_update(USB_BALONG_HOSTPMU_REMOVE);
    printk("pre pmu remove event received\n");
    if(bsp_usb_is_support_vbus_bycharger()){
        if(USB_BALONG_PERIP_INSERT == usb_get_hotplug_old_status())
        {
            bsp_host_pmu_status_change(USB_BALONG_HOSTPMU_REMOVE);
            return ;
        }
    }
...
#if((FEATURE_ON == MBB_USB_A_TO_MINI_SELECT) || (defined(BSP_CONFIG_BOARD_E5771H_937)))
        bsp_usb_status_change(
            1 == need_init ? USB_BALONG_DEVICE_INSERT : USB_BALONG_PERIP_INSERT);
#endif
#else
