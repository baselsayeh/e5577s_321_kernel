...
#define WLAN_CACHE_RECV            (65536)          /* 驱动接收缓存大小 */
#define WLAN_CACHE_CFGFILE         (20480)          /* 驱动中 config file 大小的缓存 */
...
{
    WLAN_GPIO_WIFI_WAKEUP_BB = WIFI_WAKEUP_BB_GPIO,            /* WiFi唤醒BB(单板) */
...
WLAN_STATUS_STU wlan_count = {0};    /* 用来记录错误的次数 */
...
/**********************************************************************
函 数 名  :wlan_get_product_wifi_chip_type
功能描述  : 判别单板的WiFi芯片类型
输入参数  : 无。
输出参数  : 无。
...
#if ((rtl8192 == MBB_WIFI_CHIP1) || (rtl8189 == MBB_WIFI_CHIP1))
/**********************************************************************
函数名 : wifi_kernel_cache_init
功能描述  : 申请驱动初始化所需要的连续内存
输入参数  : 无。
输出参数  : 无。
返 回 值  : WLAN_RETURN_TYPE
注意事项  : 无。
...
/**********************************************************************
函数名 : wifi_kernel_cache_free
功能描述  : 释放WIFI 初始化申请的连续内存
输入参数  : 无。
输出参数  : 无。
返 回 值  : 无。
注意事项  : 无。
