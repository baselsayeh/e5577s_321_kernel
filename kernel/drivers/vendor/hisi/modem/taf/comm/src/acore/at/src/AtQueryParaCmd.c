...
/*如果需要增加固件升级，可以添加对应的MASK宏定义
同时将对应的flag 并到FIRMWARE_UPDATA_MASK*/
...
{
/*MS2172S_818项目检测SIM_DET管脚，若为0，即表示外置SIM卡不在位ESIM卡在位，^RSIM返回0*/
#if(FEATURE_ON == MBB_FEATURE_ESIM_SWITCH)
   ...
   ulRet = NV_ReadEx(MODEM_ID_0, en_NV_Item_FollowOn_OpenSpeed_Flag, &fastboot, sizeof(VOS_UINT32));
    ...
    if(USIMM_OPENSPEEDENABLE ==  fastboot)
    {
        ...
        if (TAF_SUCCESS == AT_FillAndSndAppReqMsg(gastAtClientTab[ucIndex].usClientId,
                                                     ...
                                                     I0_WUEPS_PID_DRV_AGENT))
        {
            gastAtClientTab[ucIndex].CmdCurrentOpt = AT_CMD_SIM_QRY;           /*设置当前操作模式 */
            return AT_WAIT_ASYNC_RETURN;                                       /* 等待异步事件返回 */
        }
        else
        {
...
/*******************************************************************************
 函数名称  : ether_atoe
 功能描述  : Convert Ethernet address string representation to binary data
 输入参数  : a	string in xx:xx:xx:xx:xx:xx notation
 输出参数  : e	binary data
...
void et_macAddr_add(char *macAddr, int add_val)
{
    ...
    while (add_val--)
    {
        ...
        {   
            /*如果最后3个字节全为0xFF,则后三位变成00:00:00重新开始*/
...
VOS_UINT32 AT_QryPhyNumPara(VOS_UINT8 ucIndex)
{
...
#if (FEATURE_ON == MBB_COMMON)
    VOS_UINT8                           aucWlanMac[AT_PHYNUM_MAC_LEN + 1]; /* MAC地址*/
#endif
    /* Added by f62575 for SMALL IMAGE, 2012-1-3, end */
 ...
 || defined(BSP_CONFIG_BOARD_506HW_2)
        /*考虑为空的情况,同时还需要修改wifi mac地址个数为2,ini配置文件需要同步修改*/
        if('\0' == aucMac[0]) 
        {
            /* MAC地址输出 */
            usLength += (VOS_UINT16)At_sprintf(AT_CMD_MAX_LEN,
  ...
  || defined(BSP_CONFIG_BOARD_E5577BS_937) || defined(BSP_CONFIG_BOARD_E5573BS)
        /*考虑为空的情况,同时还需要修改wifi mac地址个数为4,ini配置文件需要同步修改*/
        if('\0' == aucMac[0]) 
        {
            /* MAC地址输出 */
            usLength += (VOS_UINT16)At_sprintf(AT_CMD_MAX_LEN,
...
#if ( FEATURE_ON == MBB_COMMON )
/*****************************************************************************
 函 数 名      : At_QryCsosPara
 功能描述  : AT^CSOS查询命令处理函数
...
VOS_UINT32 At_QryLteCat(VOS_UINT8 ucIndex)
{
    ...
    ulResult = stUECapability.enUeCatg + 1;   //NV从0开始，等级从1开始，所以+1
    gstAtSendData.usBufLen = (TAF_UINT16)At_sprintf(AT_CMD_MAX_LEN,
                                              (TAF_CHAR *)pgucAtSndCodeAddr,
                                              (TAF_CHAR*)pgucAtSndCodeAddr,
                                              "%s: %d",
...
{
    VOS_UINT32                          ulResult = NV_READ_UNABLE;
    TAF_UINT8                             ACCOUNT[2]; /*ACCOUNT有两个元素，其中一个为保留*/
