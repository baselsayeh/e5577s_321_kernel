...
/* Check whether the device is a disk or a datacard ? */
static bool sd_is_mount_disk_device(struct us_data *us)
{
    bool bRet = true;
    uint16_t uiIdVendor = 0;

    if (us)
    {
        uiIdVendor = le16_to_cpu(us->pusb_dev->descriptor.idVendor);

        /* Whether is it a datacard ? */
        if ((uiIdVendor == 0x19d2) || (uiIdVendor == 0x12d1))
        {
            bRet = false;
        }
    }
...
static int get_physicalmedium_usbdev_info(struct scsi_device *sdp, struct scsi_disk *sdkp, struct gendisk *gd)
{
    ...
    if( sdp != NULL && sdkp != NULL && gd != NULL )
    {
        ...
        if (sd_is_mount_disk_device(us) && disk != NULL)
        {
            disk->isvalid = true;
